...
function Content() {
    ...
    return (
        <div className="w-full bg-[#F6F6F5] rounded-l-2xl p-6 h-screen">
            ...
            <div className="mt-8 ml-10 grid grid-cols-1 md:grid-cols-2 gap-y-4 gap-x-8 w-full py-7  px-10 mb-10 h-[70%] overflow-y-scroll">
                {modules.map((module, index) => (
                    <div
                        key={index}
                        className={`bg-white p-5 mb-4 rounded-3xl shadow-md max-w-[370px] min-w-[330px] min-h-[220px] relative ${
                            index % 2 === 0 ? "top-[-20px]" : "top-[100px]"
                        }`}
                    >
                        ...
                        {/* Conditionally render Sone or Stwo based on index */}
                        <div className="flex-col ">
                            {index % 2 === 0 ? (
                                <img className="relative bottom-10 left-[343px]" src={Stwo} alt="Sone" />
                            ) : (
                                <img className="relative bottom-60 right-36" src={Sone} alt="Stwo" />
                            )}
                        </div>
                    </div>
                ))}
